classdef TestMVTSeriesHcatRename < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function hcat_single(tc)
            xx = tseries.MVTSeries(tseries.MIT(tseries.Unit(),1), {'a','b'}, rand(15, 2));
            r = horzcat(xx);
        ...
        function hcat_two(tc)
            U = tseries.Unit();
            xx = tseries.MVTSeries(tseries.MIT(U,1):tseries.MIT(U,15), {'a','b'}, rand(15, 2));
            yy = tseries.MVTSeries(tseries.MIT(U,3):tseries.MIT(U,10), {'c'},     rand(8, 1));
            r = [xx, yy];
            tc.verifyClass(r, 'tseries.MVTSeries');
        ...
        function vcat_appends_rows(tc)
            x = tseries.MVTSeries(tseries.qq(2020,1):tseries.qq(2020,4), {'a','b'}, [1 5; 2 6; 3 7; 4 8]);
            y = tseries.MVTSeries(tseries.qq(2021,1):tseries.qq(2021,2), {'a','b'}, [9 13; 10 14]);
            r = [x; y];
            tc.verifyClass(r, 'tseries.MVTSeries');
        ...
        function rename_with_vector(tc)
            x = tseries.MVTSeries(tseries.qq(2020,1):tseries.qq(2020,4), {'a','b','c'}, rand(4,3));
            x = rename_columns(x, {'X','Y','Z'});
            tc.verifyEqual(x.colnames, ["X","Y","Z"]);
        end

        function rename_with_struct(tc)
            x = tseries.MVTSeries(tseries.qq(2020,1):tseries.qq(2020,4), {'a','b'}, rand(4,2));
