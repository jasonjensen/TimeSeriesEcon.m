classdef TestShiftLagLead < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function shift_left(tc)
            x = tse.TSeries(tse.qq(2020,1), zeros(3,1));
            y = shift(x, 1);
            tc.verifyTrue(y.firstdate == tse.qq(2019,4));
        ...
        function shift_right(tc)
            x = tse.TSeries(tse.qq(2020,1), (1:4)');
            y = shift(x, -2);
            tc.verifyTrue(y.firstdate == tse.qq(2020,3));
        end

        function lag_default(tc)
            y = tse.TSeries(tse.MIT(tse.Unit(),2000):tse.MIT(tse.Unit(),2010), 1);
            y = cumsum(y);
            y1 = lag(y);
