classdef TestTSeriesIndex < matlab.unittest.TestCase
    ...
    methods (Test)
        function int_indexing_scalar(tc)
            t = tse.TSeries(tse.MIT(tse.Unit(),4):tse.MIT(tse.Unit(),8), rand(5,1));
        ...
        function mit_indexing(tc)
            ...
            tc.verifyEqual(s(tse.qq(2020,1)), 11);
            tc.verifyEqual(s(tse.qq(2022,4)), 22);
        end

        function mit_range_indexing_returns_tseries(tc)
            s = tse.TSeries(tse.qq(2020,1), (11:22)');
            sub = s(tse.qq(2020,1):tse.qq(2020,4));
            tc.verifyClass(sub, 'tse.TSeries');
            tc.verifyEqual(sub.values, (11:14)');
            tc.verifyTrue(sub.firstdate == tse.qq(2020,1));
        end

        function bounds_check_mit(tc)
            s = tse.TSeries(tse.qq(2020,1), (11:22)');
            tc.verifyError(@() s(tse.qq(2019,1)), 'tseries:bounds');
            tc.verifyError(@() s(tse.qq(2017,1):tse.qq(2019,12)), 'tseries:bounds');
        end
        ...
        function wrong_freq_index_throws(tc)
            s = tse.TSeries(tse.qq(2020,1), (11:22)');
            tc.verifyError(@() s(tse.MIT(tse.Unit(),1)), 'tseries:mixedFreq');
            tc.verifyError(@() s(tse.yy(2020):tse.yy(2021)), 'tseries:mixedFreq');
        end

        function assign_grows_at_left(tc)
            t = tse.TSeries(tse.mm(2018,1):tse.mm(2018,12), (1:12)');
            t(tse.mm(2017,10)) = -1;
            tc.verifyTrue(t.firstdate == tse.mm(2017,10));
        ...
        function assign_grows_at_right(tc)
            t = tse.TSeries(tse.mm(2018,1):tse.mm(2018,12), (1:12)');
            t(tse.mm(2019,2)) = -1;
            tc.verifyTrue(tse.lastdate(t) == tse.mm(2019,2));
        ...
        function assign_range_to_vector(tc)
            t = tse.TSeries(tse.mm(2018,1):tse.mm(2018,12), (1:12)');
            t(tse.mm(2019,2) : tse.mm(2019,4)) = [9 10 11];
            tc.verifyEqual(t.values(14:16), [9 10 11]');
        end

        function assign_range_to_scalar(tc)
            t = tse.TSeries(tse.qq(2020,1):tse.qq(2020,4), (1:4)');
            t(tse.qq(2020,2):tse.qq(2020,3)) = 7;
            tc.verifyEqual(t.values, [1 7 7 4]');
        end

        function logical_indexing(tc)
            t = tse.TSeries(tse.qq(2020,1), (1:5)');
            v = t(t.values > 2);
        ...
        function lastdate_for_empty(tc)
            t = tse.TSeries(tse.qq(2020,1));
            tc.verifyEqual(length(t), 0);
            % lastdate < firstdate when empty
            tc.verifyTrue(tse.lastdate(t) < t.firstdate);
        end

        function resize_keeps_old_data(tc)
            t = tse.TSeries(tse.qq(2020,1):tse.qq(2020,4), (1:4)');
            t2 = resize(t, tse.qq(2020,2):tse.qq(2021,1));
            tc.verifyTrue(t2.firstdate == tse.qq(2020,2));
