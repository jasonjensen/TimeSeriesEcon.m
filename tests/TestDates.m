classdef TestDates < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function mit2yp_daily(tc)
            tc.verifyEqual(double(tse.mit2yp(tse.daily('2022-01-01'))), [2022, 1]);
            tc.verifyEqual(double(tse.mit2yp(tse.daily('2022-01-03'))), [2022, 3]);
        end

        function mit2yp_bdaily(tc)
            tc.verifyEqual(double(tse.mit2yp(tse.bdaily('2022-01-03'))), [2022, 1]);
            tc.verifyEqual(double(tse.mit2yp(tse.bdaily('2022-01-04'))), [2022, 2]);
        end

        function bdaily_basic(tc)
            bd1 = tse.MIT(tse.BDaily(), 527256);
            tc.verifyEqual(datetime(2022,1,3), tse.toDate(bd1));
            bd2 = tse.bdaily('2022-01-03');
        ...
        function bdaily_strict_default_on_weekend_throws(tc)
            tc.verifyError(@() tse.bdaily('2022-01-02'), 'tseries:noMatch');
        end

        function bdaily_previous_next(tc)
            tc.verifyEqual(datetime(2021,12,31), ...
                tse.toDate(tse.bdaily('2022-01-02', 'bias', 'previous')));
            tc.verifyEqual(datetime(2022,1,3), ...
                tse.toDate(tse.bdaily('2022-01-02', 'bias', 'next')));
        end

        function bdaily_nearest(tc)
            % Saturday rounds to Friday
            tc.verifyTrue(tse.bdaily('2022-01-01','bias','nearest') == tse.bdaily('2021-12-31'));
            % Sunday rounds to Monday
            tc.verifyTrue(tse.bdaily('2022-01-02','bias','nearest') == tse.bdaily('2022-01-03'));
        end
        ...
        function weekly_basic(tc)
            ...
            tc.verifyTrue(tse.weekly('2022-01-01', 6) == w2);
        end
        ...
        function date_round_trip(tc)
            ...
            tc.verifyEqual(datetime(2022,1,2),   tse.toDate(tse.weekly('2022-01-01')));
            tc.verifyEqual(datetime(2021,12,27), tse.toDate(tse.weekly('2022-01-01'), 'begin'));
            tc.verifyEqual(datetime(2022,1,1),   tse.toDate(tse.weekly('2022-01-01', 6)));
        end
    end
end
