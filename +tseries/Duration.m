classdef Duration
    ...
    methods
        function obj = Duration(F, value)
            ...
            if ~isa(F, 'tseries.Frequency')
                error('tseries:noMatch', ...
                    'Duration(F, n) requires a tseries.Frequency as first argument.');
            end
            if nargin < 2
                value = 0;
            end
        ...
        function n = double(d)
            if isa(d.frequency, 'tseries.YPFrequency')
        ...
        function r = plus(a, b)
            if isa(a, 'tseries.Duration') && isa(b, 'tseries.Duration')
            ...
            elseif isa(a, 'tseries.Duration') && isnumeric(b)
                r = tseries.Duration(a.frequency, a.value + int64(b));
            elseif isnumeric(a) && isa(b, 'tseries.Duration')
                r = tseries.Duration(b.frequency, int64(a) + b.value);
            else
        ...
        function r = uminus(d)
            r = tseries.Duration(d.frequency, -d.value);
        end
        ...
        function r = times(a, b)
            if isa(a, 'tseries.Duration') && isnumeric(b)
                r = tseries.Duration(a.frequency, a.value * int64(b));
            elseif isnumeric(a) && isa(b, 'tseries.Duration')
                r = tseries.Duration(b.frequency, int64(a) * b.value);
            else
        ...
        function r = rem(a, b)
            ...
            r = tseries.Duration(a.frequency, rem(a.value, b.value));
        end
        ...
        function r = floor_div(a, b)
            ...
            r = tseries.Duration(a.frequency, idivide(a.value, b.value, 'fix'));
        end
        ...
        function tf = lt(a, b)
            ...
            elseif isnumeric(a) && isa(b, 'tseries.Duration')
                tf = (int64(a) < b.value);
            elseif isa(a, 'tseries.MIT') || isa(b, 'tseries.MIT')
                error('tseries:invalidArith', ...
