classdef MIT
    ...
    %
    %   See also: tseries.Duration, tseries.qq, tseries.mm, tseries.yy,
    %             tseries.daily, tseries.bdaily, tseries.weekly,
    %             tseries.MITRange.

    properties (SetAccess = immutable)
        value (1,1) int64       = int64(0)
        frequency               = tseries.Unit()
    end
    ...
    methods
        function obj = MIT(F, varargin)
            ...
            if ~isa(F, 'tseries.Frequency')
                error('tseries:noMatch', ...
                    'MIT(F, ...) requires a tseries.Frequency as first argument.');
            end
            obj.frequency = F;
            switch numel(varargin)
                ...
                case 2
                    y = int64(varargin{1});
                    p = int64(varargin{2});
                    obj.value = tseries.MIT.yp2value(F, y, p);
                otherwise
                    error('tseries:noMatch', ...
        ...
        function n = toFloat(m)
            ...
            % plotting.
            if isa(m.frequency, 'tseries.YPFrequency')
                yp = mit2yp(m);
        ...
        function r = plus(a, b)
            if isa(a, 'tseries.MIT') && isa(b, 'tseries.MIT')
                error('tseries:invalidArith', ...
                    'Illegal addition of two MIT values.');
            elseif isa(a, 'tseries.Duration') && isa(b, 'tseries.MIT')
                error('tseries:invalidArith', ...
                    'Illegal addition of Duration and MIT. Try MIT + Duration.');
            elseif isa(a, 'tseries.MIT') && isa(b, 'tseries.Duration')
                if ~eq(a.frequency, b.frequency)
                    mixed_freq_error(a.frequency, b.frequency);
                end
                r = tseries.MIT(a.frequency, a.value + b.value);
            elseif isa(a, 'tseries.MIT') && isnumeric(b)
                if isa(b, 'double') && any(b ~= fix(b))
                    % Float math: return float.
                    r = toFloat(a) + b;
                else
                    r = tseries.MIT(a.frequency, a.value + int64(b));
                end
            elseif isnumeric(a) && isa(b, 'tseries.MIT')
                if isa(a, 'double') && any(a ~= fix(a))
                    r = a + toFloat(b);
                else
                    r = tseries.MIT(b.frequency, int64(a) + b.value);
                end
            else
        ...
        function r = minus(a, b)
            if isa(a, 'tseries.MIT') && isa(b, 'tseries.MIT')
                ...
                r = tseries.Duration(a.frequency, a.value - b.value);
            elseif isa(a, 'tseries.MIT') && isa(b, 'tseries.Duration')
                if ~eq(a.frequency, b.frequency)
                    mixed_freq_error(a.frequency, b.frequency);
                end
        ...
        function r = rdivide(a, b)
            if isa(a, 'tseries.MIT') && isa(b, 'tseries.Duration')
                if ~eq(a.frequency, b.frequency)
                    mixed_freq_error(a.frequency, b.frequency);
                end
        ...
        function s = char(m)
            ...
            elseif isa(F, 'tseries.Yearly')
                yp = tseries.mit2yp(m);
                s = sprintf('%dY', double(yp(1)));
                if F.endPeriod ~= F.defaultEndPeriod()
                    s = [s sprintf('{%d}', F.endPeriod)];
                end
            elseif isa(F, 'tseries.HalfYearly')
                yp = tseries.mit2yp(m);
                s = sprintf('%dH%d', double(yp(1)), double(yp(2)));
                if F.endPeriod ~= F.defaultEndPeriod()
                    s = [s sprintf('{%d}', F.endPeriod)];
                end
            elseif isa(F, 'tseries.Quarterly')
                yp = tseries.mit2yp(m);
                s = sprintf('%dQ%d', double(yp(1)), double(yp(2)));
                if F.endPeriod ~= F.defaultEndPeriod()
                    s = [s sprintf('{%d}', F.endPeriod)];
                end
            elseif isa(F, 'tseries.Monthly')
                yp = tseries.mit2yp(m);
                s = sprintf('%dM%d', double(yp(1)), double(yp(2)));
            elseif isa(F, 'tseries.Daily') || isa(F, 'tseries.BDaily')
                d = mitToDate(m);
                s = char(d, 'yyyy-MM-dd');
            elseif isa(F, 'tseries.Weekly')
                d = mitToDate(m);
                s = char(d, 'yyyy-MM-dd');
            elseif isa(F, 'tseries.YPFrequency')
                yp = tseries.mit2yp(m);
